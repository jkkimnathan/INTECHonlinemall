-- ############################################################
-- ⚠️ 실행하지 마세요 (DO NOT RUN) — 보관용 참고 파일
--
-- 이 파일은 이전(클라우드) 세션에서 만든 create_order 기반 주문검증 방식입니다.
-- 현재 몰은 토스페이먼츠 실연동 방식(payment_migration.sql +
-- payment_migration_fix.sql + constraints_migration.sql)으로 대체되었고,
-- 라이브 DB에는 그쪽 함수(finalize_order_payment, qna_unlock)가 적용돼 있습니다.
--
-- 이 파일을 실행하면 아래 문제가 생깁니다:
--   • qna_unlock 이 옛 버전으로 덮여 Q&A 비밀글 열람이 깨짐(클라이언트는 {ok,code} 형식을 기대)
--   • 사용하지 않는 create_order RPC 가 다시 생성됨
-- 필요한 정책은 이미 다른 마이그레이션에 반영돼 있으니 실행 대상이 아닙니다.
-- (단, 아래 reviews_insert_verified '실구매자만 리뷰' 정책은 원하면 그 부분만
--  골라 실행할 수 있습니다.)
-- ############################################################

-- ============================================================
-- 주문 무결성 마이그레이션 v2 (미사용 · 보관용)
