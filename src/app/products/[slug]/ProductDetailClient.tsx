...
export default function ProductDetailClient({
  ...
  // persist 스토어(위시리스트) 의존 UI는 마운트 후에만 판별 (하이드레이션 불일치 방지)
  ...
  const wished = mounted && isInWishlist(product.id);
  const finalPrice = product.salePrice ?? product.price;
  const discountRate =
    product.salePrice && product.price > 0 && product.salePrice < product.price
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">
            ...
            <div>
              ...
              <div className="mt-4 p-5 bg-[#f5f5f7] rounded-xl">
                {product.salePrice && discountRate > 0 ? (
                  <>
              ...
              <div className="flex gap-2 mt-6">
                ...
                <Button
                  size="lg"
                  variant="outline"
                  className={`rounded-full ${wished ? "text-[#DC2626] border-[#fecaca]" : ""}`}
                  onClick={() => {
                ...
                >
                  <Heart className={`h-4 w-4 ${wished ? "fill-[#DC2626]" : ""}`} />
                </Button>
              </div>
